...
export const mcpClaimsRouter = router({
  ...
  // 创建应用提交
  create: protectedProcedure.input(zCreateClaimsFormSchema).mutation(async ({ input }) => {
    const app = await appsDataAccess.getById(input.appId);
    if (!app) {
      throw new Error("应用不存在");
    }
    return claimsDataAccess.create({ ...input, appName: app.name, appSlug: app.slug, appType: app.type });
  }),
