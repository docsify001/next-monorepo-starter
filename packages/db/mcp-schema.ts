...
export const ads = pgTable("ads", {
  ...
  placement: varchar("placement", { enum: ["top", "middle", "bottom"] }).default("bottom"),
...
// 推荐类型分为 app推荐，即在app详情页，推荐新的app；分类推荐：category，即在分类列表中显示的体检列表
...
export const recommendations = pgTable("recommendations", {
  ...
  type: varchar("type", { length: 20, enum: ["rank", "popular", "new", "related", "category", "app"] }).notNull(),
  title: varchar("title", { length: 255 }).notNull(),
  appCount: integer("app_count").default(0),
...
export const recommendationApps = pgTable("recommendation_apps", {
  id: text("id").primaryKey().notNull().$defaultFn(() => createId()),
  categoryId: text("category_id"), // 分类ID，如果是分类推荐，此值肯定不为空。
  recommendationId: text("recommendation_id").notNull(), // 推荐ID
  appId: text("app_id").notNull(), // 应用ID
  type: varchar("type", { length: 20, enum: ["rank", "popular", "new", "related"] }).notNull(), // 类型
  order: integer("order").default(0), // 排序
  status: varchar("status", { length: 20, enum: ["pending", "active"] }).notNull().default("pending"),
  createdAt: timestamp("created_at", { mode: "date" }).defaultNow().notNull(),
}, (table) => [
  index("recommendation_apps_category_id_idx").on(table.categoryId),
  index("recommendation_apps_app_id_idx").on(table.appId),
  uniqueIndex("recommendation_apps_unique_idx").on(table.categoryId, table.appId),
]);
...
export const appAnalysisHistory = pgTable("app_analysis_history", {
  ...
  appId: text("app_id").notNull(),
  version: varchar("version", { length: 50 }),
  sourceCode: text("source_code"), // 源码内容或URL
  ...
  status: varchar("status", { length: 50 }).notNull().default("completed"), // 分析状态
...
export const appSubmissions = pgTable("app_submissions", {
  ...
  status: varchar("status", { length: 20, enum: ["pending", "approved", "rejected", "in_review"] }).notNull().default("pending"),
  rejectionReason: text("rejection_reason"),
  reviewedAt: timestamp("reviewed_at", { mode: "date" }),
  reviewedBy: text("reviewed_by"),
...
export const recommendationAppsRelations = relations(recommendationApps, ({ one }) => ({
  category: one(categories, {
    fields: [recommendationApps.categoryId],
    references: [categories.id],
  }),
  app: one(apps, {
...
export const assets = pgTable("assets", {
  id: text("id").primaryKey().notNull().$defaultFn(() => createId()),
  userId: text("user_id").notNull(),
  assetType: varchar("asset_type", { length: 20, enum: ["avatar", "banner", "icon", "logo", "image", "document"] }).notNull(),
