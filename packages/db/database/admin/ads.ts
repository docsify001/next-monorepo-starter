...
export const adsDataAccess = {
  ...
  update: async (id: string, data: Omit<typeof zUpdateAdsSchema._type, "id">, userId: string) => {
    return db.update(ads).set(data).where(eq(ads.id, id)).returning();
  },
  ...
  delete: async (id: string) => {
    return db.delete(ads).where(eq(ads.id, id)).returning();
  },
  ...
  updateStatus: async (id: string, status: AdsStatus) => {
    return db.update(ads).set({ status }).where(eq(ads.id, id)).returning();
  },
