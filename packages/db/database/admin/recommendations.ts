...
import { zCreateRecommendationSchema, zUpdateRecommendationSchema, zSearchRecommendationsSchema, Recommendation } from "types";
...
export const recommendationDataAccess = {
  ...
  getRecommendationsByAppid: async (appId: string): Promise<Recommendation[]> => {
    return await db.select({
      ...
      appCount: recommendations.appCount,
      createdAt: recommendations.createdAt,
      recommendationApp: {
        id: recommendationApps.id,
        type: recommendationApps.type,
        order: recommendationApps.order,
        status: recommendationApps.status,
        categoryId: recommendationApps.categoryId,
      }
    })
      .from(recommendationApps)
      .innerJoin(
        recommendations,
        eq(recommendationApps.recommendationId, recommendations.id)
      )
      .where(
        and(
