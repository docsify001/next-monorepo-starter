...
export default function Home() {
  const { data, isLoading } = trpc.apiKeys.getApiKeys.useQuery();
  const { mutate, isPending } = trpc.apiKeys.createApiKey.useMutation();
    ...
    <div className="grid grow grid-rows-[20px_1fr_20px] items-center justify-items-center gap-16 p-8 pb-20 font-[family-name:var(--font-geist-sans)] sm:p-20">
      <main className="row-start-2 flex flex-col items-center gap-8 sm:items-start">
        ...
        <ol className="list-inside list-decimal text-center font-[family-name:var(--font-geist-mono)] text-sm sm:text-left">
          ...
          {data && (<li>Save and see your changes instantly.${JSON.stringify(data)}</li>)}
        </ol>

        <div className="flex flex-col items-center gap-4 sm:flex-row">
          <button onClick={() => mutate()}>Create API Key</button>
          {isPending && <li>Creating...</li>}
        </div>
